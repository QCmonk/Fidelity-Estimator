...
class random_MPUgen(object):
    ...
    def next(self):
        ...
        mpu = mp.MPArray.from_array_global(random_U(self.max_size, num=1).reshape([2]*(self.max_size*2)), ndims=2)
        ...
        while subsys>0:
            ...
            mpo_rand = mp.MPArray.from_array_global(random_U(dim, num=1).reshape([2]*(dim*2)), ndims=2)
...
def UniU_error(perturbations=100, exact=True):
    ...
    Uni_U = mporep(mpo_dict["t"], mpo_dict["id"], N-1) #np.kron(Universal_U(), np.eye(2**(N//2)))
    ...
    for i in range(0, perturbations):
        print("Now computing unitary perturbation {}\r".format(i), end="", flush=True)
        ...
        if exact:
            ...
            a_est, a_uncert = angle_estimate(stabilisers, rho_c, N=N, shots=8000)
    ...
    if exact:
        print("Average estimation error for {} perturbations: {:.3f}".format(perturbations, np.real(np.mean(error))))
    else:
...
if __name__ == '__main__':
    UniU_error(perturbations=500, exact=False)    
